...
export default {
  ...
  props: {
    validateDate: {
      // we must receive a valid trigger so DatPicker knows whether to validate
      type: Boolean,
      required: true
    }
  },
  watch: {
    ...
    validateDate: function() {
      ...
      if (this.validateDate) {
        ...
        this.$emit("reset", [false, "extra emitter message for testing"]);
        // could do this:
        // this.$emit("reset", false);
        // or just this.$emit("reset"), then use that to trigger a handler to reset
        // whatever is needed
      }
      console.log("validateDate was false, so skip running functions");
    },
    startDate: function() {
